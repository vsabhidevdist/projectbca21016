...
$elements=array(
  'appo_date'=>"",'appo_time'=>"","doctor_id"=>"","slot"=>"","user_id"=>"","booked_datetime"=>"","status"=>"");
    ...
    $d=array('date');
    $dateavailable=$dao->getDataJoin($d,'schedule','doctor_id='.$_GET['id'].' and date>=CURDATE()  order by date LIMIT 7');
    $times=array('m_start','m_end','e_start','e_end','date');
    $timeslots=$dao->getDataJoin($times,'schedule','doctor_id='.$_GET['id'].' and date>=CURDATE()  order by date LIMIT 7');
